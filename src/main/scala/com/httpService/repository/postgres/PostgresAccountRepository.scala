...
import com.httpService.domain.domain.{Account, AccountId, Balance}
...
class PostgresAccountRepository(xa: Transactor[IO]) extends AccountRepository {
  ...
  override def create(account: domain.Account): IO[Unit] = {
    sql"""
  ...
  override def find(id: AccountId): IO[Option[domain.Account]] = {
    sql"""
         SELECT id, balance
         FROM accounts
         WHERE id = ${id.value}
  ...
  override def update(account: domain.Account): IO[Unit] = {
    sql"""
         UPDATE accounts
